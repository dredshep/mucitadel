...
export default function DankBridge() {
  ...
  const openMetaDownload = () => openInNewTab('https://metamask.io/download.html')
  function MetamaskButton() {
    if (props.hasMetamask) {
      return (
        <>
          {(() => {
            if ((direction.value === 'BSC' && !ETHApproved) || (direction.value === 'ETH' && !BSCApproved)) {
              return (
                <Button onClick={handleApprove}>
                  {(() => {
                    if (direction.value === 'BSC') return 'Approve Ethereum Contract'
                    if (direction.value === 'ETH') return 'Approve BSC Contract'
                  })()}
                </Button>
              )
            } else {
        ...
        <div className="bg-asidebg rounded-xl w-full overflow-hidden max-w-max">
          <div className="text-title text-secondary text-lg font-semibold justify-around max-w-max">
            <div className="w-full flex justify-between mb-3 bg-mainbg">
              <div className="px-10 flex justify-around items-center">DANK Bridge</div>
              <div
