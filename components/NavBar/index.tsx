...
function MainLogo() {
  return <img src="/images/mucitadel-beta.svg" className="" alt="MemeUnity Logo" />
}

function Logo(props: { sidebar?: boolean }) {
  return (
    <div className={(props.sidebar ? 'ml-6' : 'ml-0') + ' flex flex-row items-center w-40 mr-auto lg:mx-0'}>
      <MainLogo />
    </div>
  )
}
...
export default function NavBar(props: {
  ...
  return (
    <div className="flex flex-row lg:justify-between h-16 bg-asidebg items-center px-6">
      ...
      <SearchBar className="hidden lg:flex" value={props.term} onChange={props.onChangeTerm} />
      <NavRightSide
        logIn={props.logIn}
        logOut={props.logOut}
